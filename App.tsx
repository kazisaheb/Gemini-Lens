...
const App: React.FC = () => {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      if (!foundImage) {
        setError("AI did not return an edited image. Try a clearer prompt.");
      }
    } catch (err: any) {
      console.error(err);
      ...
      if (err.message?.includes("Requested entity was not found")) {
        ...
        setError("AI processing failed. Please try a different preset or check your connection.");
      }
    } finally {
      ...
      <div className="min-h-screen flex items-center justify-center bg-white p-4">
        <div className="max-w-md w-full text-center space-y-8 bg-gray-50 p-10 rounded-3xl border border-gray-100 shadow-xl">
          <div className="flex justify-center">
            <div className="w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center text-white text-3xl font-bold">G</div>
          </div>
          <div className="space-y-2">
            <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Gemini Lens</h1>
            <p className="text-gray-500">Log in with your Google AI Studio account to use your personal credits.</p>
            <p className="text-xs text-gray-400">
              Note: You must select a API key from a paid GCP project. 
              See <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">billing documentation</a>.
            </p>
          </div>
          <button onClick={handleLogin} className="w-full flex items-center justify-center px-6 py-4 bg-gray-900 text-white rounded-xl hover:bg-black transition-all font-semibold shadow-lg">
